...
try:
    ...
    cv2 = None
    logging.getLogger(__name__).warning("OpenCV is not installed; video frame extraction will not work.")
...
def save_as_gif(images, output_gif_path):
    ...
    logger.info("Saved GIF: %s", output_gif_path)
...
def extract_frames_from_gif(input_gif_path, output_dir):
    ...
    with imageio.imopen(input_gif_path, "r", plugin="pillow") as gif:
        ...
        for frame in gif.iter():
            ...
            logger.info("Saved frame %d to %s", i + 1, frame_filename)
            i += 1

    logger.info("All frames extracted to %s", output_subdir)

    
if __name__ == "__main__":
    logging.basicConfig(level=logging.INFO, format="%(asctime)s - %(levelname)s - %(message)s")
    raise SystemExit("This module is a utility library. See the README and examples for usage.")
