...
# 导入自定义的 TStar 接口
from TStar.interface_yolo import YoloInterface, YoloV5Interface, YoloWorldInterface
...
class TStarSearcher:
    ...
    def spline_keyframe_distribution(
        ...
        # Lightweight interpolation (SciPy-free) to estimate scores for unvisited frames.
        all_frames = np.arange(video_length)
        interp_scores = np.interp(
            all_frames,
            frame_indices.astype(float),
            observed_scores.astype(float),
            left=float(observed_scores[0]),
            right=float(observed_scores[-1]),
        )
        ...
        adjusted_scores = np.maximum(1 / video_length, interp_scores)
    ...
    def compute_frame_relation_scores(self, frame_confidences: List[float],
        ...
        计算逻辑关系的置信度分数
        '''
        # RELATION_ALPHA = self.relation_alpha  # 已删除relations功能
        SPATIAL_ALPHA = 0.5
        ATTRIBUTE_ALPHA = 0.5
        TIME_ALPHA = 0.5
        DELTA_T = 5
        ...
        # for rel_idx, relation in enumerate(self.relations):  # 已删除relations功能
        #     # 二元关系只计算一次
    ...
    def sample_frames(self, num_samples: int) -> Tuple[List[int], np.ndarray]:
        ...
        if len(self.Score_history) == 0:  # If Score_history is empty, use uniform sampling with equal intervals
            ...
            if non_zero_entries < num_samples:
                ...
                logger.warning(
                    "Not enough non-zero entries in sampling distribution; falling back to full distribution "
                    "(need %d samples, have %d non-zero entries).",
                    num_samples,
                    non_zero_entries,
                )
    ...
    def verify_and_remove_target(
        ...
        for target in list(self.remaining_targets):
            if target in detected_objects:
                ...
                self.score_distribution[int(frame_sec)] = single_confidence

                # Visualization caches are only available when using `search_with_visualization`.
                if hasattr(self, "image_grid_iters"):
                    self.image_grid_iters.append([frame])
                if hasattr(self, "detect_annotot_iters") and hasattr(self.yolo, "bbox_visualization"):
                    self.detect_annotot_iters.append(
                        self.yolo.bbox_visualization(images=[frame], detections_inbatch=self.yolo.detections_inbatch)
                    )
                if hasattr(self, "detect_bbox_iters"):
                ...
                if target in single_detected_objects and single_confidence > confidence_threshold:
                    ...
                    logger.info("Found target '%s' in frame %d (score %.3f)", target, frame_idx, float(single_confidence))
                    return True
    ...
    def search_with_visualization(self) -> Tuple[List[np.ndarray], List[float], int]:
        ...
        # remain_unsatisfied_relation = True  # 已删除relations功能
        
        num_iterations = 0
        # 只检查剩余目标对象
        while self.remaining_targets and self.search_budget > 0:
            num_iterations += 1
            # remain_unsatisfied_relation = False  # 已删除relations功能
            # if self.relation_alpha > 0:  # 已删除relations功能
            ...
            # Sample frames based on the current distribution
            sampled_frame_secs, frames = self.sample_frames(num_frames_in_grid)
            logger.debug("Sampled frame seconds: %s", sampled_frame_secs)
            self.search_budget -= num_frames_in_grid
            ...
            # print("satisfied relationships: ", self.satisfied_relations)  # 已删除relations功能
            # print("remaining targets", self.remaining_targets)
            
            # Update the progress bar
            progress_bar.update(1)

        # Save score history for analysis (optional, uses output_dir instead of hard-coded paths).
        if self.output_dir:
            try:
                os.makedirs(self.output_dir, exist_ok=True)
                video_id = os.path.splitext(os.path.basename(self.video_path))[0]
                np.save(
                    os.path.join(self.output_dir, f"{video_id}_score_history.npy"),
        ...
        # print("satisfied relations: ", self.satisfied_relations)  # 已删除relations功能
        # Close the progress bar once the loop is done
        progress_bar.close()
        # Select top K frames based on the score distribution
        top_k_indices = np.argsort(self.score_distribution)[-K:][::-1]
...
if __name__ == "__main__":
    ...
    raise SystemExit(
        "This module is a library. Please run `examples/run_keyframe_search.py` for a runnable example."
    )