...
# Optional heavy dependencies (MMDetection stack for YOLO-World)
try:
    ...
    init_detector = None  # type: ignore[assignment]
...
try:
    ...
    ColorPalette = None  # type: ignore[assignment]

class LabelAnnotator(sv.LabelAnnotator if sv is not None else object):
...
class YoloInterface:
    ...
    def set_BBoxAnnotator(self):
        if sv is None or ColorPalette is None:
            raise ImportError("supervision is required for visualization. Install `supervision` to enable annotators.")
...
class YoloWorldInterface(YoloInterface):
    def __init__(self, config_path: str, checkpoint_path: str, device: str = "cuda:0"):
        ...
        if Config is None or Compose is None or init_detector is None:
            raise ImportError(
                "YOLO-World adapter requires the MMDetection stack. "
                "Please install `mmengine` and `mmdet` (and their dependencies)."
            )
...
class YoloV5Interface(YoloInterface):
    ...
    def inference(self, images: str, max_dets: int = 100, score_threshold: float = 0.3, use_amp: bool = False):
        ...
        """
        if sv is None:
            raise ImportError("YoloV5Interface requires `supervision` for the `sv.Detections` container.")
